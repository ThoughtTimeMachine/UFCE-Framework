...
OPT_STATE_DIR = "./opt_states" # New storage for the heavy optimizer
...
# --- GLOBAL RAM CACHE (Weights Only) ---
MODEL_RAM_CACHE = [None] * NUM_LAYERS

# --- 1. OPTIMIZER STATE MANAGER (DISK) ---
def init_opt_states_to_disk(model, dummy_input):
    """Initializes optimizer states and immediately saves them to disk to free RAM."""
    print("   💾 Initializing Optimizer States on Disk...")
    os.makedirs(OPT_STATE_DIR, exist_ok=True)
    
    # Create one template state
    dummy_params = model.init(random.PRNGKey(0), dummy_input)['params']
    base_opt_state = TX.init(dummy_params)
    
    # Clone it to disk for every layer
    for i in range(NUM_LAYERS):
        path = os.path.join(OPT_STATE_DIR, f"state_{i}.pkl")
        with open(path, 'wb') as f:
            pickle.dump(jax.device_get(base_opt_state), f)
            
    print("   ✅ Optimizer States Offloaded.")
    del base_opt_state, dummy_params
    gc.collect()

def load_opt_state(layer_idx):
    """Fetches optimizer state from disk just in time."""
    path = os.path.join(OPT_STATE_DIR, f"state_{layer_idx}.pkl")
    with open(path, 'rb') as f:
        return pickle.load(f)

def save_opt_state(layer_idx, state):
    """Writes updated state back to disk and clears it from RAM."""
    path = os.path.join(OPT_STATE_DIR, f"state_{layer_idx}.pkl")
    with open(path, 'wb') as f:
...
def save_checkpoint(layer_idx, params, step):
    ...
    flat_weights = {}
    prefix = f"model.layers.{layer_idx}."
    def to_disk(x): return np.array(x).astype(np.float16)
...
def run_training():
    print(f"🚀 VELOCITY STABLE TRAINER (Disk-Offload Optimizer)")
    print(f"🎯 8B Model | RAM Cache (Weights): ON | Disk Cache (Opt State): ON")
    print("-" * 60)
    
    model = LlamaDecoderLayer(DIM, INTERMEDIATE, HEADS, KV_HEADS)
    
    # 1. SETUP DISK OPTIMIZER (This saves 64GB RAM)
    init_opt_states_to_disk(model, jnp.ones((BATCH_SIZE, SEQ_LEN, DIM)))
    ...
    for step in range(TOTAL_STEPS):
        ...
        for i in reversed(range(NUM_LAYERS)):
            ...
            # Load Weights (RAM) and Opt State (Disk)
            layer_params = load_layer_weights(i)
            opt_state = load_opt_state(i) # <--- Fetches from SSD
            layer_input = jax.device_put(activations_store[i])
            
            # FIRST RUN JIT
            if step == 0 and i == NUM_LAYERS - 1:
                print("   (Compiling Backward Kernel... ~30s)...")
            ...
            # Save Opt State back to Disk (The Tax) and Clear RAM
            save_opt_state(i, new_opt_state)
            
            # Update RAM Cache for Weights
            ...
            del layer_params, layer_input, new_params, new_opt_state, opt_state
            gc.collect() # Force cleanup
            
            print(f"   Layer {i} BWD | {time.time()-t0:.2f}s")
